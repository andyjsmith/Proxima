...
def main():
    ...
    # -- split console view ---------------------------------------------
    from proxima.ui.split import MAX_PANES as SPLIT_MAX

    window.panes.gather()
    for key in list(window.consoles):
        window.close_console(key)
    window.panes.focus_page(window.summary)
    pump(0.4)
    if window.panes.pane_count() != 1:
        failures.append(
            f"the window did not start with one pane: "
            f"{window.panes.pane_count()}")

    split_a = FakeConsole("split-a")
    split_a.guest_key = f"{CONN_ID}/pve-node-01/qemu/100"
    split_b = FakeConsole("split-b")
    split_b.guest_key = f"{CONN_ID}/pve-node-01/qemu/101"
    for console in (split_a, split_b):
        window.consoles[console.guest_key] = console
        window.panes.append(console, Gtk.Label(label=console.title))
    pump(0.4)

    # Splitting moves the console in front into a pane of its own, rather
    # than opening an empty one nothing could be dragged into.
    window.panes.focus_page(split_b)
    pump(0.3)
    window._split_console()
    pump(0.4)
    if window.panes.pane_count() != 2:
        failures.append(f"splitting gave {window.panes.pane_count()} panes")
    elif window.panes.notebook_of(split_b) is window.panes.notebook_of(split_a):
        failures.append("the split console stayed in the same pane")
    elif not window.panes.notebook_of(split_b).get_visible():
        failures.append("the new pane is not visible")
    elif window.current_console() is not split_b:
        failures.append(
            f"the split console is not the current one: "
            f"{window.current_console()}")
    else:
        print("[smoke] splitting moves a console into a second, visible pane")

    # Both consoles are still known, and the session records both panes.
    if len(window.panes.all_pages()) != 3:      # summary + two consoles
        failures.append(f"pages went missing: {window.panes.all_pages()}")
    config["restore_session"] = True
    window._save_session()
    if len(config["session_consoles"]) != 2:
        failures.append(
            f"a split session did not record both consoles: "
            f"{config['session_consoles']}")
    else:
        print("[smoke] a split session records consoles from every pane")

    # Splitting is refused when a pane holds only one tab -- the pane would
    # simply move, leaving an empty one behind.
    window._sync_view_menu()
    if window.split_item.get_sensitive():
        failures.append("splitting a lone tab out of its pane is offered")
    else:
        print("[smoke] splitting a pane's last tab is not offered")

    # A pane that loses its last page hides itself again.
    window.panes.remove_page(split_b)
    pump(0.4)
    if window.panes.pane_count() != 1:
        failures.append(
            f"an emptied pane stayed on screen: {window.panes.pane_count()}")
    else:
        print("[smoke] a pane with nothing left in it closes itself")

    # Up to four, and no further.
    window.panes.append(split_b, Gtk.Label(label="split-b"))
    extras = []
    for index in range(SPLIT_MAX + 1):
        console = FakeConsole(f"extra-{index}")
        console.guest_key = f"extra-{index}"
        extras.append(console)
        window.panes.append(console, Gtk.Label(label=console.title))
    pump(0.3)
    for console in extras:
        window.panes.focus_page(console)
        window.panes.split(console)
    pump(0.4)
    if window.panes.pane_count() != SPLIT_MAX:
        failures.append(
            f"panes went past the limit: {window.panes.pane_count()}")
    else:
        print(f"[smoke] the split view stops at {SPLIT_MAX} panes")

    # Gathering brings everything back into one pane without losing a page.
    before = len(window.panes.all_pages())
    window.panes.gather()
    pump(0.4)
    if window.panes.pane_count() != 1:
        failures.append("gathering left panes open")
    elif len(window.panes.all_pages()) != before:
        failures.append(
            f"gathering lost pages: {before} -> "
            f"{len(window.panes.all_pages())}")
    elif window.panes.notebook_of(split_a) is not window.panes.primary:
        failures.append("gathering did not return consoles to the first pane")
    else:
        print("[smoke] gathering returns every console to one pane")

    for console in extras:
        window.panes.remove_page(console)
    for console in (split_a, split_b):
